...
{
	{.Kp = 12.5,.Ki = 0.1,.Kd = 0.5,.Max = 16000,.Min = -16000,.IntegralLimit = 1000,.DeadBand = 10},	//ID = 1
	{.Kp = 12.5,.Ki = 0.1,.Kd = 0.5,.Max = 16000,.Min = -16000,.IntegralLimit = 1000,.DeadBand = 10},	//ID = 2
	{.Kp = 12.5,.Ki = 0.05,.Kd = 0.5,.Max = 16000,.Min = -16000,.IntegralLimit = 1000,.DeadBand = 10},	//ID = 3
	{.Kp = 17,.Ki = 2,.Kd = 3,.Max = 16000,.Min = -16000,.IntegralLimit = 1000,.DeadBand = 10},	 	//ID = 4
	{.Kp = 12.5,.Ki = 0.1,.Kd = 0.5,.Max = 16000,.Min = -16000,.IntegralLimit = 1000,.DeadBand = 10},	 	//ID = 5
...
{
	{.Kp = 0.25,.Ki = 0.01,.Kd = 0,.Max = 14000,.Min = -14000,.IntegralLimit = 20000,.DeadBand = 100},	//ID = 10
	...
	{.Kp = 10,.Ki = 0,.Kd =0 ,.Max = 14000,.Min = -14000,.IntegralLimit = 20000,.DeadBand = 0},	//ID = 4
...
路径修正pid（非必要）
*********************************************************************************/
M3508_PID M3508_Rot_Pid[2] = 
{
	{.Kp = 10,.Ki = 0.01,.Kd = 10,.Max = PI,.Min = -PI,.IntegralLimit = 20000,.DeadBand = 0.5},
	{.Kp = 15,.Ki = 0.01,.Kd = 10,.Max = PI,.Min = -PI,.IntegralLimit = 20000,.DeadBand = 0.5}	
};
//采样时间在5~10ms
...
int speed_level;
/*********************************************************************************
  *@  name      : RM3508_Set_Rotate
  *@  function  : 小车旋转角度设置（非必要）
  *@  input     : 目标角度
  *@  output    : 无
*********************************************************************************/
float RM3508_Set_angle(float aim_angle)
{
	aim_angle = aim_angle * 2 * PI /360;
//	err_angle = err_angle * 2 * PI /360;
	float err_angle = gyro_data.Z_Pos * 2 * PI /360;
//	float V_X = speed * sin(aim_angle);
//	float V_Y = speed * cos(aim_angle);

	M3508_Rot_Pid[speed_level].PID_Out_last = M3508_Rot_Pid[speed_level].PID_Out;//？
	M3508_Rot_Pid[speed_level].Err_last = M3508_Rot_Pid[speed_level].Err;
	M3508_Rot_Pid[speed_level].Err 	=  aim_angle - err_angle;

	//死区 if
	M3508_Rot_Pid[speed_level].Err_sum += M3508_Rot_Pid[speed_level].Err; 
	M3508_Rot_Pid[speed_level].Err_sum = CLAMP(M3508_Rot_Pid[speed_level].Err_sum,-1000,1000);
	
	M3508_Rot_Pid[speed_level].P_Out = M3508_Rot_Pid[speed_level].Kp * M3508_Rot_Pid[speed_level].Err;
	M3508_Rot_Pid[speed_level].I_Out = M3508_Rot_Pid[speed_level].Ki * M3508_Rot_Pid[speed_level].Err_sum;
	M3508_Rot_Pid[speed_level].D_Out = M3508_Rot_Pid[speed_level].Kd * (M3508_Rot_Pid[speed_level].Err - M3508_Rot_Pid[speed_level].Err_last); 
	
	M3508_Rot_Pid[speed_level].PID_Out = M3508_Rot_Pid[speed_level].P_Out + M3508_Rot_Pid[speed_level].I_Out + M3508_Rot_Pid[speed_level].D_Out;
//	M3508_Rot_Pid.PID_Out = CLAMP(M3508_Rot_Pid.PID_Out,M3508_Rot_Pid.Min,M3508_Rot_Pid.Max);

	float omega = M3508_Rot_Pid[speed_level].PID_Out - M3508_Rot_Pid[speed_level].PID_Out_last;
 // float VZ_W=omega*length;
//	flag = omega;
...
/*陀螺仪偏移角度计算*/
//double angle2cnt = gyro_data.Z_Pos * PI / 180;
double	angle2cnt =0;
/*解算到机器人坐标系运动向量*/
     int VX = (int)(-((float)sin(angle2cnt)) * VY_W +((float)cos(angle2cnt)) * VX_W);
     int VY = (int)(((float)cos(angle2cnt)) * VY_W + ((float)sin(angle2cnt)) * VX_W);
    int VZ =0;
		// int VZ = RM3508_Set_angle(aim_angle)*Kz;
	//	printf("VX:%f,  VY:%f  ",VX,VY);
		
	RM3508_Set_Speed((-(VY - VX )*Kv+VZ) +(Rotate_Vx*Kv),1);
	RM3508_Set_Speed(((VY + VX )*Kv+VZ)+(Rotate_Vx*Kv),2);	
	RM3508_Set_Speed(((VY - VX )*Kv+VZ)+(Rotate_Vx*Kv),3);	
  RM3508_Set_Speed((-(VY + VX)*Kv+VZ)+(Rotate_Vx*Kv),5);
...
{
		RM3508_Set_Speed(Rotate_Vx,1);
		RM3508_Set_Speed(Rotate_Vx,2);
		RM3508_Set_Speed(Rotate_Vx,3);
		RM3508_Set_Speed(Rotate_Vx,5); 
